...
export const voiceInputAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        const inventorySummary = inventory.length > 0
            ? inventory.slice(0, 10).map((i) => `${i.name} (${i.expiry_label})`).join(", ")
            : "no inventory yet";
  ...
  "confidence": number (0-1)
        ...
        try {
            ...
            if (match) {
                ...
                await callback({ text: aiResponse });
            }
        }
        catch (err) {
    ...
    examples: [
        [
            { user: "user", content: { text: "what can I cook today", source: "voice", transcript: "what can I cook today" } },
            { user: "KitchenCopilot", content: { text: "Looking at your inventory, I'd suggest making a stir-fry tonight! Your vegetables are expiring in 2 days and would be perfect with the chicken you have." } }
        ],
        [
            { user: "user", content: { text: "I used the eggs", source: "voice", transcript: "I used the eggs" } },
            { user: "KitchenCopilot", content: { text: "Got it! Removing eggs from your inventory now. Your meal plan has been updated to use alternative breakfast options." } }
        ]
    ],
};
