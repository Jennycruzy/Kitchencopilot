...
export const budgetPlanningAction = {
    name: "PLAN_BUDGET",
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        if (!budget && text) {
            ...
            if (dollarMatch)
                extractedBudget = parseFloat(dollarMatch[1]);
            else if (numberMatch)
                extractedBudget = parseFloat(numberMatch[1]);
        }
...
Give a warm 1-2 sentence confirmation and a helpful tip about maximizing their budget.`;
    ...
    examples: [
        [
            { user: "user", content: { text: "Set my budget to $75 per week" } },
            { user: "KitchenCopilot", content: { text: "✅ Budget set to $75/week! I'll prioritize your existing inventory and only add the most cost-effective items to your shopping list." } }
        ]
    ],
};
//# sourceMappingURL=budgetPlanning.js.map