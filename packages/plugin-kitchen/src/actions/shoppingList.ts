...
export const shoppingListAction: Action = {
...
[{"name":"item","estimated_cost":1.99,"unit":"each/lb/bag","store_section":"produce/dairy/meat/pantry/frozen"}]`;
        ...
        try {
            ...
            const items = match ? JSON.parse(match[0]) : needed.map((n: string) => ({ name: n, estimated_cost: 0 }));
            const total = items.reduce((s: number, i: any) => s + (i.estimated_cost || 0), 0);
    ...
    examples: [
        [
            { name: "user", content: { text: "Add milk to my shopping list" } },
            { name: "KitchenCopilot", content: { text: "🛒 Added milk to your shopping list! Estimated cost: $3.49" } }
        ]
    ] as ActionExample[][],
};
